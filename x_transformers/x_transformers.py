...
class XTransformer(nn.Module):
    ...
    @torch.no_grad()
    def get_encoding(self, src, tgt, src_mask = None, tgt_mask = None, src_attn_mask = None):
        enc = self.encoder(src, mask = src_mask, attn_mask = src_attn_mask, return_embeddings = True)
        return enc

    @torch.no_grad()
    def generate_from_encoding(self, encodings, seq_out_start, seq_len, src_mask = None, **kwargs):
        return self.decoder.generate(seq_out_start, seq_len, context = encodings, context_mask = src_mask, **kwargs)
